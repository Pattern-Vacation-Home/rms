...
class PropertyController extends Controller
{
    ...
    public function dtcmPermits(Request $request)
    {
        ...
        $today = today();
        $query = $this->dtcmPermitQuery($status, $search, $buildingId, $ownerId);
    ...
    public function dtcmPermitsExcel(Request $request)
    {
        $filters = $this->validatedDtcmFilters($request);
        $properties = $this->dtcmPermitQuery($filters['status'], $filters['search'], $filters['building_id'], $filters['owner_id'])
            ->orderBy('name')->get();

        return response()->streamDownload(function () use ($properties) {
            $output = fopen('php://output', 'wb');
            fwrite($output, "\xEF\xBB\xBF");
            fputcsv($output, ['DTCM Permit Report']);
            fputcsv($output, ['Generated by', 'System']);
            fputcsv($output, ['Generated at', now()->format('d M Y H:i')]);
            fputcsv($output, []);
            fputcsv($output, ['Unit', 'Building', 'Owner', 'Permit Number', 'Issue Date', 'Expiry Date', 'Days Remaining', 'Status', 'Reminder']);

            foreach ($properties as $property) {
                $permit = $property->dtcmDocuments->first();
                $details = $this->dtcmPermitDetails($permit);
                fputcsv($output, [
                    $property->name,
                    $property->building?->building_name ?? 'No building',
                    $property->landlord?->name ?? 'Not assigned',
                    $permit?->reference_no ?? '',
                    $permit?->issue_date?->format('Y-m-d') ?? '',
                    $permit?->expires_at?->format('Y-m-d') ?? '',
                    $details['days_label'],
                    $details['status_label'],
                    $permit?->expiry_reminder_sent_for?->equalTo($permit?->expires_at) ? 'Sent' : 'Not sent',
                ]);
            }
            fclose($output);
        }, 'dtcm-permit-report-' . now()->format('Y-m-d') . '.csv', ['Content-Type' => 'text/csv; charset=UTF-8']);
    }

    public function dtcmPermitsPdf(Request $request)
    {
        $filters = $this->validatedDtcmFilters($request);
        $properties = $this->dtcmPermitQuery($filters['status'], $filters['search'], $filters['building_id'], $filters['owner_id'])
            ->orderBy('name')->get();

        return PdfRenderer::downloadView('admin.properties.pdf.dtcm-permits', compact('properties', 'filters'),
            'dtcm-permit-report-' . now()->format('Y-m-d') . '.pdf', ['format' => 'A4-L']);
    }

    private function validatedDtcmFilters(Request $request): array
    {
        $request->validate([
            'status' => 'nullable|in:valid,expiring,urgent,expired,missing',
    ...
    private function dtcmPermitQuery(string $status = '', string $search = '', ?string $buildingId = null, ?string $ownerId = null)
    {
        $today = today();

        return Property::with(['building', 'landlord', 'dtcmDocuments'])
    ...
    private function dtcmPermitDetails($permit): array
    {
        $days = $permit?->expires_at ? today()->diffInDays($permit->expires_at, false) : null;
        $status = ! $permit ? 'missing' : (! $permit->expires_at ? 'no_expiry' : ($days < 0 ? 'expired' : ($days <= 7 ? 'urgent' : ($days <= 30 ? 'expiring' : 'valid'))));

        return [
            'days' => $days,
            'days_label' => is_null($days) ? '—' : ($days < 0 ? abs((int) $days) . ' overdue' : (int) $days . ' days'),
            'status' => $status,
