            ...
            <div class="card overflow-hidden">
                <div class="card-body">
                    <div class="d-flex align-items-center justify-content-between gap-2">
                        ...
                        <div class="text-end"><span class="badge {{ ['paid'=>'bg-success','partial'=>'bg-warning','unpaid'=>'bg-danger'][$paymentSummary['status']] ?? 'bg-secondary' }} text-white">{{ $paymentSummary['status']==='partial' ? 'Partly paid' : ucfirst($paymentSummary['status']) }}</span><div class="small text-muted mt-1">{{ str($booking->status)->replace('_',' ')->headline() }}</div></div>
                    </div>
                    <div class="row mt-3 g-2">
                        ...
                        <div class="col-12"><p class="mb-0 small">Received AED {{ number_format($paymentSummary['paid'],2) }} · Balance AED {{ number_format($paymentSummary['balance'],2) }}</p></div>
                    </div>
                </div>
