...
@php($doorLock = $booking->property?->smartlock)
@if($doorLock)
<div class="card mb-3" style="border:1px solid #d9e2f8;border-radius:14px;overflow:hidden"><div class="card-body"><div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-3"><div><h4 class="card-title mb-1"><i class="ri-door-lock-line text-primary me-1"></i>Guest Door Access</h4><div class="small text-muted">{{ $doorLock->alias ?: $doorLock->name }} · Passcodes follow paid invoice periods only.</div></div><span class="badge {{ $doorLock->has_gateway && (int)$doorLock->passcode_version===4 ? 'bg-success-subtle text-success' : 'bg-warning-subtle text-warning' }}">{{ $doorLock->has_gateway ? 'Gateway connected' : 'Gateway required' }} · V{{ $doorLock->passcode_version ?? '—' }}</span></div>
<div class="d-flex flex-wrap gap-2">@foreach($booking->invoices->sortBy('period_from') as $accessInvoice)@php($issuedAccess = $booking->lockAccesses->firstWhere('booking_invoice_id',$accessInvoice->id))@if($issuedAccess)<div class="border rounded-3 px-3 py-2"><strong class="d-block small">{{ $accessInvoice->invoice_number }}</strong><span class="small {{ $issuedAccess->revoked_at ? 'text-danger' : 'text-success' }}">{{ $issuedAccess->revoked_at ? 'Revoked' : 'Issued until '.$issuedAccess->ends_at->copy()->timezone('Asia/Dubai')->format('d M H:i') }}</span>@if(!$issuedAccess->revoked_at)<form method="POST" action="{{ route('admin.smartlocks.revoke',$issuedAccess) }}" onsubmit="return confirm('Revoke this guest door code now?');">@csrf<button class="btn btn-link btn-sm text-danger p-0">Revoke access</button></form>@endif</div>@elseif(!$accessInvoice->legacy_owner_settled && $accessInvoice->balance_due <= 0 && $booking->status !== 'checked_out')<form method="POST" action="{{ route('admin.smartlocks.issue',$booking) }}">@csrf<input type="hidden" name="invoice_id" value="{{ $accessInvoice->id }}"><button class="btn btn-outline-primary" @disabled(!$doorLock->has_gateway || (int)$doorLock->passcode_version!==4)><i class="ri-key-line me-1"></i>Issue · {{ $accessInvoice->period_from?->format('d M') }}–{{ $accessInvoice->period_to?->format('d M') }}</button></form>@endif @endforeach</div><div class="small text-muted mt-3">Access is visible to the linked tenant only during its paid stay period. Do not share the master lock code.</div></div></div>
@endif
<div class="row">
        ...
        <form action="{{ route('admin.booking-invoice.payment', $invoice->id) }}" method="POST" enctype="multipart/form-data">@csrf
            ...
            <div class="modal-body">
                <div class="row g-3 mb-3">
                    ...
                    <div class="col-md-6"><label class="form-label">Payment Method</label><select name="payment_method" class="form-select" required><option>Bank Transfer</option><option>Cash Deposit</option><option>Cash</option><option>Card</option><option>Cheque</option><option>Online Payment</option></select></div>
